...
const up = async(knex) => {
    const log = logger('DB: trigger-migration-up');
    log.info('Trigger migration up operation initiated');

    const triggerFunction = await readSqlFile('triggers/functions', 'update_version');
    const trigger = await readSqlFile('triggers', 'triggers');

    knex.raw(triggerFunction).then(() => {
